...
class CustomUser(AbstractUser, BaseModel):
    ...
    def generate_code(self, verify_type):   #user.generate_code(verify_type)
...
class CodeVerify(BaseModel):
    ...
    def save(self, *args, **kwargs):
        if not self.expire_time:
            expiration = EMAIL_EXPIRATION_TIME if self.verify_type == VIA_EMAIL else PHONE_EXPIRATION_TIME
            self.expire_time = timezone.now() + timedelta(minutes=expiration)
